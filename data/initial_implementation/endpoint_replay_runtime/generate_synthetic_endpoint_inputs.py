...
TOPIC_BLOCKS = [
    ...
    },
    {
        ...
        "utterances": [
            "The endpoint replay runtime should be independently runnable from a fresh virtual machine.",
...
def setup_logger(log_file: Path | None = None) -> logging.Logger:
    logger = logging.getLogger("generate_synthetic_endpoint_inputs")
...
def build_stage1_request_rows(meeting_id: str, utterances: list[dict], window_size: int, transition_index: int) -> list[dict]:
    rows: list[dict] = []
...
def main() -> None:
    ...
    for meeting_id in meeting_ids:
        ...
        write_json(
            manifest_path,
            {
                ...
                "source_type": "synthetic_endpoint_replay",
    ...
    write_json(
        manifest_root / f"v{args.version}.json",
        {
            ...
            "generator": "generate_synthetic_endpoint_inputs.py",
        },
    )

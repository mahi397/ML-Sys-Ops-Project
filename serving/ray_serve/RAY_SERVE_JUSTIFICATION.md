# Ray Serve Integration — Bonus Item Justification
...
3. **Deployment composition:**
   ...
   ```
   The full pipeline (Stage A → Stage B) is expressed as a composition of
   deployments. Ray Serve handles the internal routing and load balancing.
...
- **Ray Serve:** Multi-model pipelines where models need independent scaling
  and resource isolation — exactly our use case
